...
class Metric:
    ...
    def calculate_masked_digit_score(predictions, targets, masks, penalty_factor=0.9):
        """
        Calculate the score for each output sentence based on the matching positions of masked digits,
        with a penalty factor applied when a non-digit is predicted for masked values.

        Args:
            predictions (List[str]): List of predicted output sentences.
            targets (List[str]): List of target output sentences.
            masks (List[List[int]]): List of masks indicating the positions of masked digits in each target sentence.
            penalty_factor (float): Penalty factor to apply when a non-digit is predicted for masked values.

        Returns:
            float: Average score across all output sentences.
        """
        total_score = 0
...
class T5Model(pl.LightningModule):
    ...
    # Freezing some of the layers may be helful when we are training on a small dataset and it is not very different from pretrained dataset
    ...
    def _step(self, batch):  ## loss function
...
class TextData(Dataset):
    ...
    def mask_numerical_digits(self, sentence):
        ...
        num_digits_to_mask = int(0.5 * len(digits_to_mask))
    ...
    def convert_to_features(self, example_batch):
        ...
        input_encoding = self.tokenizer.encode_plus(question,
                                                    max_length=self.input_length,
                                                    padding='max_length',
                                                    truncation=True,
                                                    return_tensors='pt')
        
        output_encoding = self.tokenizer.encode_plus(masked_answer,
        ...
        mask = mask[:self.output_length]
        mask += [0] * (self.output_length - len(mask))
        return input_encoding, output_encoding, mask

    def __getitem__(self, index):
        input_encoding, output_encoding, mask = self.convert_to_features(self.dataset.iloc[index])

        source_ids = input_encoding["input_ids"].squeeze()
        source_mask = input_encoding["attention_mask"].squeeze()
        target_ids = output_encoding["input_ids"].squeeze()
        target_mask = output_encoding["attention_mask"].squeeze()
...
class TextDataModule(pl.LightningDataModule):
    def __init__(self, train_file, val_file, test_file, tokenizer_name_or_path, input_length, output_length, batch_size=16, num_workers=16):
        super().__init__()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='google/t5-v1_1-small')
    ...
    log_dir = os.path.join('logs', 'T5_masked_small')
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    
    # create a logger instance
    logger = TensorBoardLogger(save_dir='logs/', name='T5_masked_small')
       
    # Save the model and configuration
    checkpoint_dir = 'checkpoints/T5_large_digit_small/'

    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
    checkpoint_callback = ModelCheckpoint(
        dirpath=checkpoint_dir,
        filename='T5_masked_large-{epoch:02d}-val_loss={val_loss:.2f}',
        save_top_k=1,
        monitor='val_loss',
        mode='min',
    ...
    data_module = TextDataModule(train_file = 'gsm_train.csv', val_file = 'gsm_val.csv', test_file = 'gsm_val.csv', tokenizer_name_or_path = args.tokenizer_name_or_path, input_length= args.max_input_length, output_length= args.max_output_length, batch_size=16, num_workers=6)
    #set_seed(42)
    model = T5Model(args)
