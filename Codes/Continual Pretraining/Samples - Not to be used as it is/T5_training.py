...
class T5Model(pl.LightningModule):
    ...
    def _generative_step(self, batch) :        
        ...
        self.log('em_score', em_score)
        self.log('subset_match_score', subset_match_score)
        return base_metrics    
...
class TextData(Dataset):
    def __init__(self, df, tokenizer, input_length, output_length, num_samples,print_text=False):
        self.dataset = df
...
class TextDataModule(pl.LightningDataModule):
    def __init__(self, train_file, val_file, test_file, tokenizer_name_or_path, input_length, output_length, num_samples =100, batch_size=6, num_workers=7):
        super().__init__()
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_data = TextData(train_df, self.tokenizer, self.input_length, self.output_length, self.num_samples)
            self.val_data = TextData(val_df, self.tokenizer, self.input_length, self.output_length, self.num_samples)
        if stage == 'test' or stage is None:
            test_df = pd.read_csv(self.test_file)
            self.test_data = TextData(test_df, self.tokenizer, self.input_length, self.output_length, self.num_samples)
...
if __name__ == '__main__':
    ...
    data_module = TextDataModule(train_file = 'train_context.csv', val_file = 'train_context.csv', test_file = 'train_context.csv', tokenizer_name_or_path = args.tokenizer_name_or_path, input_length= args.max_input_length, output_length= args.max_output_length, num_samples = 100, batch_size=8, num_workers=0)
    #set_seed(42)
    model = T5Model(args)
