...
class Metric:
    ...
    @staticmethod
    def calculate_em_score_loss(predicted_spans, actual_spans):
        total_values = 0
        exact_match_count = 0

        for predicted, actual in zip(predicted_spans, actual_spans):
            predicted_values = predicted.split(',')[0].split()
            actual_values = actual.split(',')[0].split()
            for value in actual_values:
                if value.strip() in predicted_values:
                    exact_match_count += 1
                total_values += 1

        em_score_loss = exact_match_count / total_values
        return em_score_loss
...
class T5Model(pl.LightningModule):
    ...
    def _step(self, batch):
        ...
        text_span = self.ids_to_clean_text(lm_labels)
        ...
        # Calculate the span loss
        predicted_spans = outputs.logits.argmax(dim=-1)  # the predicted spans are obtained from the logits
        predicted_text_span = self.ids_to_clean_text(predicted_spans)
        
        '''
        intersection = len(set(text_span) & set(predicted_text_span))
        union = len(set(text_span) | set(predicted_text_span))
        jaccard_similarity = intersection / union

        # Calculate the F1 score
        f1 = f1_score(text_span, predicted_text_span, average='micro')

        # Combine the cross-entropy loss and other loss
        loss = ce_loss + (1-f1) + 1.5 * (1-jaccard_similarity)
        '''
        
        em_score_loss = 1 - Metric.calculate_em_score_loss(predicted_text_span, text_span)

        loss = ce_loss + em_score_loss

        return loss, ce_loss, em_score_loss
    ...
    def _generative_step(self, batch):
        ...
        loss, ce_loss, em_score_loss = self._step(batch)
        ...
        self.log("val em_score_loss", em_score_loss, sync_dist=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train em_score_loss", em_score_loss, on_step=False, on_epoch=True, sync_dist=True)
...
class TextData(Dataset):
    ...
    def convert_to_features(self, example_batch):
        ...
        mask = self.mask_numeric_spans(text, mask_ratio = 50, noise_span_length = 1)
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join('logs', 'T5_span_loss_large')
    ...
    checkpoint_dir = 'checkpoints/T5_span_loss_large_v2/'
