...
class TextData(Dataset):
    ...
    def split_into_segment(self, ds, input_length):
        ...
        for index, row in ds.iterrows():
            if len(row['text'].split()) > input_length:
        ...
        ds2 = pd.DataFrame(new_rows, columns=['text'])
        ds = ds.append(ds2)
        return ds
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger(save_dir='logs/', name='T5_loss_baseline')
       
    # Save the model and configuration
    checkpoint_dir = 'checkpoints/T5_loss_baseline/'
